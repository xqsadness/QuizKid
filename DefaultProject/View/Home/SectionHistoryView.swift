            ...
            VStack{
                ZStack {
                    Image("Ellipse")
                        ...
                        .overlay {
                            Circle()
                                .trim(from: 0, to: CGFloat(point.first?.pointHistory ?? 0) / CGFloat(CONSTANT.SHARED.DATA_HISTORY.count))
                                .stroke(Color.text, style: StrokeStyle(lineWidth: 5, lineCap: .round))
                                .rotationEffect(.degrees(-90))
                        }
                    
                    Text("\(String(format: "%.0f", CGFloat(point.first?.pointHistory ?? 0) / CGFloat(CONSTANT.SHARED.DATA_HISTORY.count) * 100 ))%")
                        .font(.bold(size: 12))
                        .foregroundColor(Color.text)
                }
