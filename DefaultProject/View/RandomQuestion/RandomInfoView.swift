...
struct RandomInfoView: View {
    ...
    var body: some View {
        HStack{
            VStack{
                Text("\(selectedTab + 1) \("of".cw_localized) \(dataListen.count)")
                    .font(.bold(size: 16))
                    .foregroundColor(Color.background)
                ProgressView(value: min(max(progress, 0), Double(dataListen.count - 1)), total: Double(dataListen.count - 1))
            }
